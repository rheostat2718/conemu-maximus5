...
public:
	...
	bool bRemote;
	wchar_t sRemoteServer[MAX_PATH+2], /*sRemoteLogin[MAX_PATH], sRemotePassword[MAX_PATH],*/ sRemoteResource[MAX_PATH*2];
	bool bDirty;
public:
	MRegistryBase();
...
public:
	...
	virtual LPBYTE GetExportBuffer(DWORD cbSize) { return NULL; };
	...
	// Wrappers
	virtual LONG CreateKeyEx(HKEY hKey, LPCWSTR lpSubKey, DWORD Reserved, LPWSTR lpClass, DWORD dwOptions, REGSAM samDesired, LPSECURITY_ATTRIBUTES lpSecurityAttributes, PHKEY phkResult, LPDWORD lpdwDisposition, DWORD *pnKeyFlags, RegKeyOpenRights *apRights = NULL, LPCWSTR pszComment = NULL) VIRTUAL_FUNCTION;
	virtual LONG OpenKeyEx(HKEY hKey, LPCWSTR lpSubKey, DWORD ulOptions, REGSAM samDesired, PHKEY phkResult, DWORD *pnKeyFlags, RegKeyOpenRights *apRights = NULL) VIRTUAL_FUNCTION;
	virtual LONG CloseKey(HKEY hKey) VIRTUAL_FUNCTION;
...
public:
	...
	virtual BOOL EditKeyPermissions(RegPath *pKey, RegItem* pItem) {return FALSE;}; // = 0;
};
