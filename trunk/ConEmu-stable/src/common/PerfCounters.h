...
public:
	// *pProcessName - имя БЕЗ расширения!
	...
	// asExeName - имя БЕЗ расширения!
	int GetProcessesByName(LPCWSTR asExeName, DWORD* pPids, int cMaxPidCount)
	{
		int iFound = 0;
			...
			for (i=0; i<(DWORD)pObj->NumCounters; i++)
			{
				if (pCounterDef->CounterNameTitleIndex == dwProcessIdTitle)
				{
					...
					for (i = 0; i < (DWORD)pObj->NumInstances; i++)
					{
						PPERF_COUNTER_BLOCK pCounter = (PPERF_COUNTER_BLOCK)((DWORD_PTR)pInst + pInst->ByteLength);
						DWORD dwProcessId = *((LPDWORD)((DWORD_PTR)pCounter + dwProcessIdCounter));
						LPCWSTR pszName = (LPCWSTR)((DWORD_PTR)pInst + pInst->NameOffset);

						if (lstrcmpiW(asExeName, pszName) == 0)
						{
							if (pPids && (cMaxPidCount > iFound))
							{
								pPids[iFound] = dwProcessId;
							}
							iFound ++;
						}
