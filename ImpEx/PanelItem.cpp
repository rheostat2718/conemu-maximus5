...
MPanelItem* MPanelItem::FindChild(LPCTSTR asItem, DWORD_PTR anUserData)
{
	MPanelItem* pChild;
...
int MPanelItem::getFindData(struct PluginPanelItem **pPanelItem,int *pItemsNumber,int OpMode)
{
	...
	*pPanelItem = (PluginPanelItem*)calloc(nChildren+1,sizeof(PluginPanelItem));
	...
	while (pChild && nCount <= nChildren) {
		#ifdef _UNICODE
		(*pPanelItem)[nCount].FindData.lpwszFileName = pChild->item.cFileName;
		(*pPanelItem)[nCount].FindData.nFileSize = pChild->item.nFileSizeLow;
		#else
		...
		#endif
		(*pPanelItem)[nCount].FindData.dwFileAttributes = pChild->item.dwFileAttributes;
		(*pPanelItem)[nCount].Description = pChild->pszDescription;
		(*pPanelItem)[nCount].Owner = pChild->pszOwner;

		(*pPanelItem)[nCount].UserData = (DWORD_PTR)pChild;
	...
	// + DUMP.TXT
	#ifdef _UNICODE
	(*pPanelItem)[nCount].FindData.lpwszFileName = DUMP_FILE_NAME;
	(*pPanelItem)[nCount].FindData.nFileSize = pszText ? (wcslen(pszText)+1)*2 : 0; // + BOM
	#else
	...
	#endif
	(*pPanelItem)[nCount].FindData.dwFileAttributes = FILE_ATTRIBUTE_NORMAL;
...
void MPanelItem::getPanelModes ( struct OpenPluginInfo *Info )
{
	if (nPanelModes) {
...
void MImpEx::getOpenPluginInfo ( struct OpenPluginInfo *Info )
{
...
int MImpEx::getFiles( struct PluginPanelItem *PanelItem, int ItemsNumber, int Move, TCHAR *DestPath, int OpMode )
{
	...
	for (int i=0; i<ItemsNumber; i++) {
		...
		lsPath[nLen-1] = _T('\\');
		lstrcpy(lsPath+nLen, FILENAMEPTR(PanelItem[i].FindData));

		LPCTSTR pszName = FILENAMEPTR(PanelItem[i].FindData);

		MPanelItem* pItem = lstrcmpi(pszName, DUMP_FILE_NAME) ?
			pCurFolder->FindChild(pszName, PanelItem[i].UserData) : pCurFolder;
		if (pItem) {
