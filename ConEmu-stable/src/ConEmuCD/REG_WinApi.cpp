...
// Ctor
MRegistryWinApi::MRegistryWinApi(BOOL abWow64on32)
...
{
	MRegistryBase* pDup = new MRegistryWinApi(mb_Wow64on32);
	if (bRemote)
...
LONG MRegistryWinApi::DeleteSubkeyTree(HKEY hKey, LPCWSTR lpSubKey)
{
	...
	if (hRc == 0)
	{
		...
		//REGFILETIME ft = {{0}};
		RegPath subkey = {RE_UNDEFINED}; subkey.Init(RE_WINAPI, mb_Wow64on32, hSubKey); // , L"", ft);
		subkeys.Init(&subkey);
