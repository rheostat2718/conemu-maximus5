...
int TranslateKeyToVK(int Key,int &VirtKey,int &ControlState,INPUT_RECORD *Rec)
{
	...
	if (!KeyInTable)
	{
 		...
 		//	VirtKey=FKey-KEY_FKEY_BEGIN;
			...
			VirtKey=FKey; // изыски, ConEmu может послать такое событие, чтобы в Far не провалилс€ Alt (его "отпускание) после нажати€ AltTab
		else if (FKey && FKey < WCHAR_MAX)
		{
			short Vk = VkKeyScan(static_cast<WCHAR>(FKey));
#ifdef _DEBUG
			// Ќа некоторых системах, почему-то обламываетс€ VkKeyScan на '[', после этого
