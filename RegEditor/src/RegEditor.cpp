		...
		case aImportRegFile:
			{
				...
				MRegistryWinApi winReg(cfg->getWow64on32());
			...
			if (!(PanelItemAttributes(PanelItem[i]) & FILE_ATTRIBUTE_DIRECTORY))
			{
				...
				if (DetectFileFormat(pszUnc, &nFormat, NULL, NULL))
				{
					if (nFormat == 0 || nFormat == 1)
					{
						...
						//		break;
						//}
						MFileReg fileReg(pPlugin->getWow64on32());
						...
						if (hRc == 0)
						{
							...
							else
							{
								...
								if (pszRootKey && IsRegPath(pszRootKey, &hkRoot, &pszSubKey, FALSE) && pszSubKey)
								{
									...
									pathFrom.Init(RE_REGFILE, pPlugin->getWow64on32(), hkRoot, pszSubKey);
								}
								else
								{
									InvalidOp();
									...
									SafeFree(pszRootKey);
									break;
									pathFrom.Init(RE_REGFILE, pPlugin->getWow64on32());
								}
								bool lbFailed = false;
				...
				if (nImportStyle == ris_AsRaw)
				{
					...
					MFileTxtReg txt(pPlugin->getWow64on32());
	...
	)
{
	...
	if ((_OpMode & OPM_FIND) == OPM_FIND)
	{
		...
		if (pItem && pItem->nMagic == REGEDIT_MAGIC && pItem->ptrData)
		{
			...
			MFileTxtReg file(pPlugin->getWow64on32());
						...
						)
{
	...
	if (pPlugin->mb_ShowRegFileMenu)
	{
		...
		else
		{
			...
			MRegistryWinApi winReg(pPlugin->getWow64on32());
...
#ifdef _UNICODE
int WINAPI ProcessSynchroEventW(
#if FAR_UNICODE>=1900
